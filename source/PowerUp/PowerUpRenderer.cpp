#include "PowerUpRenderer.h"

PowerUpRenderer::PowerUpRenderer(Shader &shader, glm::mat4 maze_modelmat, pair<float, float> PowerUp_coord) {
    this->Model = maze_modelmat;
    this->shader = shader;
    this->coordinates = {PowerUp_coord.first + 0.025, PowerUp_coord.second + 0.03};
    this->initRenderData();
}
...
void PowerUpRenderer::initRenderData() {
    // configure VAO/VBO
    unsigned int VBO;
...
void PowerUpRenderer::DrawPowerUp(Texture2D &texture) {
    this->shader.Use();
